...
void			move_forward(t_wolf *wolf, const int *keys)
{
	...
	min_dist = 0.2;
	new_x = SDL_cos(wolf->player.a) * (keys[SDL_SCANCODE_LSHIFT]
			? 0.0050 : 0.0025) * ft_time()->delta;
	new_y = SDL_sin(wolf->player.a) * (keys[SDL_SCANCODE_LSHIFT]
			? 0.0050 : 0.0025) * ft_time()->delta;
	if (new_x < 0)
		min_dist *= -1;
	if (!is_wall(wolf->player.x + new_x + min_dist,
			wolf->player.y, wolf->map.map))
		wolf->player.x += new_x;
	min_dist = 0.2;
	if (new_y < 0)
		min_dist *= -1;
	if (!is_wall(wolf->player.x, wolf->player.y + new_y +
			min_dist, wolf->map.map))
		wolf->player.y += new_y;
}
